...
def main():
    ...
    parser.add_argument('--train_batch',type=int,default=10,help='train batch size',metavar='N')
    parser.add_argument('--test_batch',type=int,default=10,help='test batch size',metavar='N')
    parser.add_argument('--lr','--learning_rate',default=0.00025,type=float,help='initial learning rate',metavar='LR')
    ...
    parser.add_argument('--resume',default = './checkpoint/FSRNetwork_47.pth.tar',type=str)
...
def train(train_loader, model, criterion_mse, criterion_cross_entropy,criterion_landmark,optimizer, epoch, use_cuda,train_batch,lr):
    ...
    for batch_idx, [batch_lr_img, batch_sr_img, batch_lbl, batch_landmark] in enumerate(train_loader):
        ...
        # pdb.set_trace() 
        loss = (criterion_mse(out_sr, batch_sr_img) + criterion_mse(coarse_out, batch_sr_img) + \
        ...
        #loss = (criterion_mse(landmark_out,batch_landmark)+criterion_cross_entropy(parsing_out,batch_lbl))/(2.0*train_batch)
        ...
        if count% 300 ==0:
            ...
            #visualize.save_image(landmark=random_landmark,parsing=random_parsing,epoch=epoch,if_train=True,count=int(count/300))        
        bar.next()
...
def test(test_loader, model, criterion_mse,criterion_cross_entropy, criterion_landmark ,optimizer, epoch, use_cuda,test_batch):
    ...
    for batch_idx, [batch_lr_img, batch_sr_img, batch_lbl, batch_landmark] in enumerate(test_loader):
        with torch.no_grad():
            ...
            # compute output
            coarse_out, out_sr, out_landmark, out_lbl = model(batch_lr_img)
            loss = (criterion_mse(out_sr, batch_sr_img) + criterion_mse(coarse_out, batch_sr_img) + criterion_landmark(out_landmark,batch_landmark) + criterion_cross_entropy(out_lbl, batch_lbl))/(2.0*test_batch)
            ...
            #train prior estimation network
            #out,landmark_out,parsing_out = model(batch_sr_img)
            #loss = (criterion_mse(landmark_out,batch_landmark)+criterion_cross_entropy(parsing_out,batch_lbl))/(2.0*test_batch)
            losses.update(loss.data, batch_lr_img.size(0))
            ...
            if count%30 ==0:
                ...
                random_img, random_landmark, random_parsing,random_coarse = out_sr[0], out_landmark[0], out_lbl[0] , coarse_out[0]
                random_img, random_landmark, random_parsing ,random_coarse= random_img.detach().cpu().numpy(), random_landmark.detach().cpu().numpy(), random_parsing.max(dim=0)[1].detach().cpu().numpy() , random_coarse.detach().cpu().numpy()
                visualize.save_image(random_coarse,random_img, random_landmark, random_parsing, epoch,if_train=False,count=int(count/30))
                ...
                #random_landmark = random_landmark.detach().cpu().numpy()
                #random_parsing = random_parsing.max(dim=0)[1].detach().cpu().numpy()
                #visualize.save_image(landmark=random_landmark,parsing=random_parsing,epoch=epoch,if_train=False,count=int(count/90))        
